...
describe("Phase 6 (a): cross-tenant isolation via the SAME shared taxonomy row", () => {
  it("Org A's evidence pack, filtered to a CQCKeyQuestion both orgs tag, contains ZERO Org B records", async () => {
    ...
    // The actual leak assertion: no Org B record id anywhere in Org A's pack.
    // (Title check omitted: both demo orgs legitimately share "Infection
    // Control Audit" as the audit type, so a title-text check would be a
    // false positive. The ID check is the correct isolation boundary.)
  ...
  it("a directly-inserted tag with Org A's own orgId but pointing entityId at Org B's RiskEntry cannot surface Org B content in Org A's pack", async () => {
    ...
    // WORKFORCE_GOVERNANCE is used here (not RISK_MANAGEMENT) because the
    // seed already creates a RISK_MANAGEMENT SixPillarTag for the Org B
    // risk entry, and the unique constraint on (entityType, entityId,
    // sixPillarId) would reject a duplicate.
    const sixPillar = await rawPrisma.sixPillar.findFirstOrThrow({
      where: { name: "WORKFORCE_GOVERNANCE" },
    });
...
describe("Phase 6: end-to-end cross-tenant PDF byte content check", () => {
  it("Org A's generated PDF bytes contain zero Org B identifiers, and vice versa", async () => {
    ...
    // PDFKit hex-encodes content stream text (e.g. "A" → "<41>") so plain
    // org name strings are not greppable there. The PDF's Info dictionary
    // (Title/Subject — set in pdf.ts via the `info` constructor option) IS
    // written as plain literal strings and is reliably grep-able in the
    // raw bytes. Check via those fields.
    //
    // Sanity: each org's OWN identifiers appear in its pack's Info dict.
    expect(pdfAText).toContain("Greenfield Aesthetic Clinic"); // in Title
    expect(pdfAText).toContain(ORG_A);                        // in Subject
    expect(pdfBText).toContain("Riverside Dental Practice");   // in Title
    expect(pdfBText).toContain(ORG_B);                        // in Subject

    // Cross-tenant isolation at the byte level: the foreign org's
    // identifiers must appear NOWHERE in the generated bytes.
